...
const matchNumber = R.when(R.is(String), R.compose(R.complement(R.isEmpty), R.match(/^-?\d*\.?\d*$/)));
...
const resolveStyles = container => R.compose(transformUnits$1(container), transformStyles, expandStyles, resolveMediaQueries$1(container), flatten);
/**
...
const resolveZIndex = node => R.evolve({
...
const layoutText = R.compose(R.reduce(R.concat, []), R.converge(engine, [getAttributedString, getContainer, getLayoutOptions])); // TODO: Do it memoizable?
...
const mergeStyles$1 = styles => R.evolve({
...
const removeMargins = R.compose(setMargin(0), R.dissocPath(['style', 'margin']), R.dissocPath(['style', 'marginTop']), R.dissocPath(['style', 'marginRight']), R.dissocPath(['style', 'marginBottom']), R.dissocPath(['style', 'marginLeft']), R.dissocPath(['style', 'marginHorizontal']), R.dissocPath(['style', 'marginVertical']));
...
const getWrap = R.ifElse(R.anyPass([isNote, isImage, isCanvas]), R.always(false), R.pathOr(true, ['props', 'wrap']));
...
const setHeight$1 = R.assocPath(['box', 'height']);
...
const breakNode = height => node => {
  ...
  const currentNode = R.compose(setChildren(currentChildren), setHeight$1(Math.ceil(offset / height) * height))(node);
  const nextNode = R.compose(setChildren(nextChildren), setHeight$1(node.box.height - offset))(node);
...
const getTop$2 = R.pathOr(0, ['box', 'top']);
const getHeight$1 = R.path(['box', 'height']);
const getChildren$1 = R.propOr([], 'children');
const isElementOutside = R.useWith(R.lte, [R.identity, getTop$2]);
...
const getPaddingBottom$1 = R.pathOr(0, ['style', 'paddingBottom']);
...
const resolvePageWrapping = R.compose(restorePageSize, resolvePageSplitting, resolvePageBreaks);
...
const resolvePagePaddings = page => R.evolve({
  style: R.evolve({
...
const getSource$1 = R.compose(getURL, R.either(R.path(['props', 'src']), R.path(['props', 'href'])));
...
const getDocumentProp = target => (or, prop) => R.pathOr(or, ['children', 0, 'props', prop], target);
...
const isNotText$1 = R.complement(isText);

const renderNode = ctx => node => {
  const renderChildren = R.tap(R.compose(R.forEach(renderNode(ctx)), R.pathOr([], ['children'])));
...
const renderDocument = ctx => R.compose(R.forEach(R.compose(renderRulers$1(ctx), renderNode(ctx))), R.pathOr([], ['children']));

const renderRoot = ctx => R.compose(R.forEach(renderDocument(ctx)), R.pathOr([], ['children']));
