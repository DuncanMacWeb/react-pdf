...
var matchNumber = when(is(String), compose(complement(isEmpty), match(/^-?\d*\.?\d*$/)));
...
var getSource = compose(when(is(String), function (src) {
  return {
    uri: src
  };
}), either(path(['props', 'src']), path(['props', 'source'])));
...
var expandedKeys = keys(styleShorthands);
/**
...
var resolveStyles = function resolveStyles(container) {
  return compose(transformUnits$1(container), transformStyles, expandStyles, resolveMediaQueries$1(container), flatten);
};
/**
...
var resolvePageSize = function resolvePageSize(key) {
  return function (page) {
    ...
    return evolve((_R$evolve = {}, _R$evolve[key] = merge(__, size), _R$evolve))(page);
  };
};
/**
 * Resolves page sizes
  ...
  return function (value) {
    return tap(function (node) {
      ...
      if (!isNil(value) && yogaNode) {
        var hasEdge = complement(isNil)(edge);
...
var layoutText = compose(reduce(concat, []), converge(engine, [getAttributedString, getContainer, getLayoutOptions])); // TODO: Do it memoizable?
...
var getMax = reduce(max, -Infinity);
/**
...
var isLayoutElement = both(isNotText, isNotNote);
...
var resolveTextLayout = function resolveTextLayout(node) {
  ...
  }), when(shouldLayoutText, compose(converge(assoc('lines'), [converge(layoutText, [identity, path(['box', 'width']), path(['box', 'height'])]), identity]))))(node);
};
...
var removeMargins = compose(setMargin(0), dissocPath(['style', 'margin']), dissocPath(['style', 'marginTop']), dissocPath(['style', 'marginRight']), dissocPath(['style', 'marginBottom']), dissocPath(['style', 'marginLeft']), dissocPath(['style', 'marginHorizontal']), dissocPath(['style', 'marginVertical']));
...
var getWrap = ifElse(anyPass([isNote, isImage, isCanvas]), always(false), pathOr(true, ['props', 'wrap']));
...
var getPaddingBottom = pathOr(0, ['style', 'paddingBottom']);
...
var setChildren = assoc('children');
var setHeight$1 = assocPath(['box', 'height']);
var setBreak = assocPath(['props', 'break']);
...
var breakNode = function breakNode(height) {
  return function (node) {
    ...
    var currentNode = compose(setChildren(currentChildren), setHeight$1(Math.ceil(offset / height) * height))(node);
    var nextNode = compose(setChildren(nextChildren), setHeight$1(node.box.height - offset))(node);
...
var resolvePageBreaks = evolve({
  children: map(evolve({
    children: compose(flatten$1, map(breakPage))
  }))
});

var getTop$2 = pathOr(0, ['box', 'top']);
var getHeight$1 = path(['box', 'height']);
var getChildren$1 = propOr([], 'children');
var isElementOutside = useWith(lte, [identity, getTop$2]);
...
var splitNode = curryN(2, function (height, node) {
...
var getPaddingBottom$1 = pathOr(0, ['style', 'paddingBottom']);
...
var resolvePageSplitting = evolve({
  children: map(evolve({
    children: compose(flatten$1, map(splitPage))
  }))
});

var restorePageSize = evolve({
  children: map(evolve({
    children: map(resolvePageSize('box'))
  }))
});
var resolvePageWrapping = compose(restorePageSize, resolvePageSplitting, resolvePageBreaks);
...
var resolveAbsoluteCoordinates = function resolveAbsoluteCoordinates(node) {
  var top = pathOr(0, ['box', 'top'], node);
  var left = pathOr(0, ['box', 'left'], node);
  return evolve({
...
var getSource$1 = compose(getURL, either(path(['props', 'src']), path(['props', 'href'])));
...
var getDocumentProp = function getDocumentProp(target) {
  return function (or, prop) {
    return pathOr(or, ['children', 0, 'props', prop], target);
  };
};
...
var isNotText$1 = complement(isText);

var renderNode = function renderNode(ctx) {
  return function (node) {
    var renderChildren = tap(compose(forEach(renderNode(ctx)), pathOr([], ['children'])));
...
var renderDocument = function renderDocument(ctx) {
  return compose(forEach(compose(renderRulers$1(ctx), renderNode(ctx))), pathOr([], ['children']));
};

var renderRoot = function renderRoot(ctx) {
  return compose(forEach(renderDocument(ctx)), pathOr([], ['children']));
};
...
export default dom;
export { Document$1 as Document, BlobProvider, PDFViewer, PDFDownloadLink, pdf, View, Text, Link, Page, Font$1 as Font, Note, Image, Canvas, version, StyleSheet, CanvasViewer };
